...
if [[ "${TORCH_VERSION}" == "2.9.0" ]]; then
  export CIBW_BEFORE_BUILD="pip install ninja wheel setuptools && pip install --pre torch==${TORCH_VERSION} --index-url https://download.pytorch.org/whl/nightly/${CUDA_VERSION}"
  export CIBW_BEFORE_TEST="pip install pytest && pip install --pre torch==${TORCH_VERSION} --index-url https://download.pytorch.org/whl/nightly/${CUDA_VERSION}"
else
