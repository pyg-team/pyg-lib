...
TORCH_LIBRARY(pyg, m) {
  m.def("pyg::grouped_matmul_autograd(Tensor[] input, Tensor[] other) -> Tensor[]", grouped_matmul_autograd);
  m.def("pyg::segment_matmul_autograd(Tensor input, Tensor ptr, Tensor other) -> Tensor", segment_matmul_autograd);
}
