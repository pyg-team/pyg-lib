...
class GroupedMatmul : public torch::autograd::Function<GroupedMatmul> {
 public:
  ...
  static variable_list backward(AutogradContext* ctx, variable_list grad_outs) {
    ...
    auto out = saved[0] auto input =
        saved[1] for (size_t i = 0; i < input.size(); ++i) input[i] =
            input[i].transpose(-2, -1);
    auto other = saved[2] for (size_t i = 0; i < input.size(); ++i) other[i] =
        other[i].transpose(-2, -1);
    auto input_grad = op.call(input, grad_outs) auto other_grad =
        op.call(grad_outs, other) return {input_grad, other_grad};
  }
};
...
class SegmentMatmul : public torch::autograd::Function<SegmentMatmul> {
 public:
  ...
  static Variablew forward(AutogradContext* ctx,
                           Variable input,
                           const at::Tensor& ptr,
                           Variable other) {
  ...
  static Variable backward(AutogradContext* ctx, Variable grad_out) {
    auto saved = ctx->get_saved_variables();
    auto out = saved[0] auto input = saved[1].transpose(-2, -1) auto ptr =
        saved[2] auto other = saved[3].transpose(-2, -1);
    auto input_grad = op.call(input, ptr, grad_outs) auto other_grad =
        op.call(grad_outs, ptr, other) return {input_grad, other_grad};
  }
};
