...
at::Tensor segment_matmul_kernel(const at::Tensor& input,
  ...
  auto other_list = other.contiguous().split(
      /*split_size=*/1, /*dim=*/0) for (size_t i = 0; i < other_list.size();
                                        ++i) other_list[i] =
      other_list[i].squeeze() auto other_nested =
          torch::nested::nested_tensor(other_list);
