...
private:
    // We have to use the relative address here as different process have different address spaces, when attaching the shared memory to the running process
    // The relative is calculated by: (current process addr - head_addr) 
    ...
    char* SerializeADictOfTensors(void* shm_ptr, const std::map<std::string, torch::Tensor> &dict_of_tensors);
    ...
    size_t FindByteSizeOfDictOfTensors(const std::map<std::string, torch::Tensor> &dict_of_tensors);
...
public:
    ...
    // Main functions
    void WriteADictOfTensors(const std::map<std::string, torch::Tensor> &dict_of_tensors, size_t data_starting_addr);
...
// A wrapper on top of SHMMessageQueue to allow the pickling/unpickling processing for mp.Process
class PicklableSHMMessageQueue{
    private:
        std::unique_ptr<SHMMessageQueue> shm_queue; // It will be used by the creator of the SHM 
        SHMMessageQueue* controller_in_shm_ptr;     // It will be used by the instances in mp.Process
        size_t data_starting_addr_per_process;      // It stores the starting address of the attached shared memory (exclude the controller/SHMMessageQueue) 
                                                    // for that particular process. The absolute value will be varied for different process as each 
                                                    // process has a new address space

    public:
        int shmid;
        PicklableSHMMessageQueue(size_t shmsize);
        PicklableSHMMessageQueue(int shmid, bool for_python_mp);  // Used in mp.Process during pickling and unpickling

        void WriteDictOfTensors(const std::map<std::string, torch::Tensor> &dict_of_tensors);
