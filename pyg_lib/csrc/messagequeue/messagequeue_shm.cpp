...
namespace pyg_distributed{
    ...
    SHMMessageQueue::SHMMessageQueue(std::size_t shm_size){
        ...
        if (shm_handle == -1) {std::cout << "Error: shmget in shm_messagequeue.cpp";}
        shmid = shm_handle; // record the SHM id for retrieval
        void* shmat_ptr = shmat(shm_handle, NULL, 0);
        if (*(int*) shmat_ptr == -1) {std::cout << "Error: shmat in shm_messagequeue.cpp";}
    ...
    inline void SHMMessageQueue::RecordANewData(std::size_t addr){
        ...
        if (address_book_write_idx.load() + 1 >= MAX_SAMPLE_NUM){address_book_write_idx.store(0);}
        else{address_book_write_idx.store(address_book_write_idx.load() + 1);}
        sem_post(&address_book_sem);
    }

    // a helper function to find the size of the serialized/1D tensor
    inline size_t SHMMessageQueue::FindByteSizeOfTensor(const torch::Tensor &tensor){
        ...
        total_size += tensor.sizes().size()*sizeof(std::int64_t); // shape values field
    ...
    inline size_t SHMMessageQueue::FindByteSizeOfString(const std::string &str){
        return str.size() + sizeof(size_t); //the size of the string and the  data size field of the string
    }

    // a helper function to find the byte size of a dictionary of tensors (the number of key-pairs can be varied)
    inline size_t SHMMessageQueue::FindByteSizeOfDictOfTensors(const std::map<std::string, torch::Tensor> &dict_of_tensors){
        size_t total_size = 0;
        total_size += sizeof(size_t);  // the data size field of this block of data
        total_size += sizeof(size_t);  // the field recording the number of key-value pairs
    ...
    char* SHMMessageQueue::SerializeAString(void* shm_ptr, const std::string &str) {
        ...
        *reinterpret_cast<size_t*>(shm_ptr) = reinterpret_cast<char*>(writer_ptr) - reinterpret_cast<char*>(shm_ptr); 
        return writer_ptr;
    }

    char* SHMMessageQueue::SerializeATensor(void* shm_ptr, const torch::Tensor &tensor) {
        // init
        char* writer_ptr = static_cast<char*>(shm_ptr);
        ...
        *reinterpret_cast<std::uint8_t*>(writer_ptr) = 1;
        ...
        std::size_t total_bytes_for_shape = tensor_shape.size() * sizeof(std::int64_t); // c10::IntArrayRef is ArrayRef<int64_t>
        ...
        *reinterpret_cast<std::size_t*>(writer_ptr) = payload_size;
        ...
        // write the size of this data
        *reinterpret_cast<std::size_t*>(shm_ptr) = reinterpret_cast<std::size_t>(writer_ptr) - reinterpret_cast<std::size_t>(shm_ptr); 
        
        return writer_ptr;
    }

    char* SHMMessageQueue::SerializeADictOfTensors(void* shm_ptr, const std::map<std::string, torch::Tensor> &dict_of_tensors){
        // init
        char* writer_ptr = static_cast<char*>(shm_ptr);
        ...
        *reinterpret_cast<size_t*>(writer_ptr) = dict_of_tensors.size();
        ...
        for (const auto& item : dict_of_tensors){
            writer_ptr = SerializeAString(reinterpret_cast<void*>(writer_ptr), item.first);
            writer_ptr = SerializeATensor(reinterpret_cast<void*>(writer_ptr), item.second);
        }
    ...
    void SHMMessageQueue::WriteADictOfTensors(const std::map<std::string, torch::Tensor> &dict_of_tensors, size_t data_starting_addr){
        sem_wait(&write_sem);
        ...
        // Check the data size
        try {
            if (data_size > size_of_shm) throw data_size; // Throw an exception when a problem arise
        }
            catch (size_t data_size) {
            std::cout << "ERROR: The data size of the message (" <<  data_size << ") " <<  "is larger than the shared memory (" << size_of_shm  << ").";
        }
        // Recommend a better shared memory size
        if (data_size*4 > size_of_shm){
            std::cout << "Warning: It is recommended to initialize a larger shared memory size for the MessageQueue." << std::endl;
        }

        // Note: In here we only allow writing happens when the writer leads
        //       This will help to remove the deadlock issue. (There is a case that writer is waiting for reader and reader is waiting for writer)
        // i.e. writer leads and remaining space is enough to write the data
        if ((writer_addr_relative.load() >= reader_addr_relative.load()) && (writer_addr_relative.load() + data_size <= shm_end_addr_relative)){
            writer_ptr = reinterpret_cast<void*>(data_starting_addr + writer_addr_relative.load());
            ...
            // Not enough space in the remaining space. Need to go back to the beginning of the buffer to write.
            // Wait for the reader to consume all the data first. This can prevent data corruption
    ...
    py::dict SHMMessageQueue::ReadADictOfTensors(size_t data_starting_addr){
        ...
        if (address_book_read_idx.load() + 1 >= MAX_SAMPLE_NUM){ // reset the reader index if that is larger than the size of the address book array
            address_book_read_idx.store(0);
        }
        else{
            address_book_read_idx.store(address_book_read_idx.load() + 1);
        }
        void* actual_readaddr_ptr = reinterpret_cast<void*>(data_starting_addr + read_addr_offset); 
        sem_post(&address_book_sem);
        ...
        // update metadata
        std::uint8_t data_size = *reinterpret_cast<std::size_t*>(actual_readaddr_ptr); // read the size of the data (which is specified at the beginning of the bytes [data_size][tensor data......])
    ...
    inline std::string SHMMessageQueue::DeserializeAString(void* shm_ptr){
        ...
        size_t str_size = *reinterpret_cast<size_t*>(reader_ptr) - sizeof(size_t); // minus the total size field
    ...
    inline torch::Tensor SHMMessageQueue::DeserializeATensor(void* shm_ptr){
        ...
        if (indicator==1){
            // shape number and values
            std::size_t torch_shape_array_size = *reinterpret_cast<std::size_t*>(reader_ptr);
            reader_ptr += sizeof(std::size_t);
            torch::IntArrayRef tensor_shapes{reinterpret_cast<std::int64_t*>(reader_ptr), torch_shape_array_size};
            reader_ptr += torch_shape_array_size * sizeof(std::int64_t);

            // data type and length
            torch::ScalarType data_type = *reinterpret_cast<torch::ScalarType*>(reader_ptr);
            ...
            reconstructed_tensor = torch::from_blob(reader_ptr, tensor_shapes, torch::TensorOptions().dtype(data_type).device(torch::kCPU)); // creates a tensor view of the data
        ...
        return reconstructed_tensor.clone().detach(); // return a new instance (as the shm buffer will be overwrote)
    }
    ...
    py::dict SHMMessageQueue::DeSerializeADictOfTensors(void* shm_ptr){
        ...
        for (size_t i = 0; i < num_of_keyvalue_pair; i++){
            ...
            torch::Tensor value = DeserializeATensor(reinterpret_cast<void*>(reader_ptr));
            reader_ptr += *reinterpret_cast<size_t*>(reader_ptr);
    ...
    PicklableSHMMessageQueue::PicklableSHMMessageQueue(std::size_t shmsize){
        try{
            ...
            std::cout << "ERROR: The shared memory size should be larger than 128MB (i.e. 134217728). It is now: " << shmsize << std::endl;
            return;
        }
        ...
        data_starting_addr_per_process += sizeof(SHMMessageQueue);        // Avoid the serializer/deserializer corrupt the controller/SHMMessageQueue

        std::cout <<"Initialized a SHM. SHM ID: " << shmid << std::endl;
    }
    
    // This constructor will be called when passing this object through mp.Process(.... args(this_object)).
    // It will use the 'shmid' and 'attach_addr' to construct a pointer of SHMMessageQueue. The pointer will point at the shared memory.
    // This is to ensure that multiprocesses are using the same controller/SHMMessagequeue to read/write tensors.
    // We leverage the mechanism of py::pickle to achieve this and avoid the limitations posed by mp.Process().
    PicklableSHMMessageQueue::PicklableSHMMessageQueue(int _shmid, bool for_python_mp){
        void* shm_ptr = shmat(_shmid, (void *)0, 0);
        if (*(int*) shm_ptr == -1) {std::cout << "Error: shmat in PicklableSHMMessageQueue";}
        shmid = _shmid;
    ...
    void PicklableSHMMessageQueue::WriteDictOfTensors(const std::map<std::string, torch::Tensor> &dict_of_tensors){ // Csat the Python dictionary to a C++ type for processing
        return controller_in_shm_ptr->WriteADictOfTensors(dict_of_tensors, data_starting_addr_per_process);
    }

    py::dict PicklableSHMMessageQueue::ReadDictOfTensors(){ // Map the dictionary to a C++ type for processing
        return controller_in_shm_ptr->ReadADictOfTensors(data_starting_addr_per_process);
    }

    // for monitoring the operation of the message queue
    void PicklableSHMMessageQueue::Monitor(){
        std::cout << "case1 : " << controller_in_shm_ptr->case1.load() << " case2 : " << controller_in_shm_ptr->case2.load() << std::endl;
    }
