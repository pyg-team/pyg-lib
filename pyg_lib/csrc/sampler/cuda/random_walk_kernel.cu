...
__global__ void random_walk_kernel_impl(
    ...
    scalar_t* __restrict__ node_data,
    scalar_t* __restrict__ edge_data,
    int64_t num_seeds,
    int64_t walk_length) {
  CUDA_1D_KERNEL_LOOP(scalar_t, i, num_seeds) {
    ...
    for (scalar_t j = 0; j < walk_length; ++j) {
      ...
      if (row_end - row_start > 0) {
        ...
        auto edge_idx = row_start + scalar_t(rand * (row_end - row_start));
        v = col_data[edge_idx];
        edge_data[j * num_seeds + i] = edge_idx;
      }
...
std::tuple<at::Tensor, at::Tensor> random_walk_kernel(const at::Tensor& rowptr,
  ...
  // Ensure contiguous access by transposing output matrices:
  const auto node_seq = rowptr.new_empty({walk_length + 1, seed.size(0)});
  const auto edge_seq = rowptr.new_full({walk_length, seed.size(0)}, -1);
  const auto rand =
  ...
  AT_DISPATCH_INTEGRAL_TYPES(seed.scalar_type(), "random_walk_kernel", [&] {
    ...
    random_walk_kernel_impl<<<blocks(seed.size(0)), threads(), 0, stream>>>(
        rowptr_data, col_data, seed_data, rand_data, node_data, edge_data,
        seed.size(0), walk_length);

    C10_CUDA_KERNEL_LAUNCH_CHECK();
  });

  return std::make_tuple(node_seq.t().contiguous(), edge_seq.t().contiguous());
}
