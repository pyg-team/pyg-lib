...
def knn_graph(
    ...
    r"""Computes a k-nearest neighbors graph for a point cloud.

    Args:
        x: Point positions of shape :obj:`[N, D]`.
        k: Number of nearest neighbors.
        batch: Batch vector of shape :obj:`[N]`.
        loop: If :obj:`True`, includes self-loops.
        cosine: If :obj:`True`, uses cosine distance (CUDA only).

    Returns:
        Edge indices of shape :obj:`[2, N*k]`.
    """
    if batch is None:
        ptr = torch.tensor([0, x.size(0)], dtype=torch.long, device=x.device)
    else:
        num_batches = int(batch.max().item()) + 1
        counts = batch.bincount(minlength=num_batches)
        ptr = torch.zeros(num_batches + 1, dtype=torch.long, device=x.device)
        torch.cumsum(counts, dim=0, out=ptr[1:])

    edge_index = knn(x, x, k if loop else k + 1, ptr, ptr, cosine)

    if not loop:
        mask = edge_index[0] != edge_index[1]
        edge_index = edge_index[:, mask]
        # Keep only k per query node:
        row = edge_index[0]
        count = torch.zeros(x.size(0), dtype=torch.long, device=x.device)
        keep = torch.zeros(edge_index.size(1), dtype=torch.bool,
                           device=x.device)
...
def radius_graph(
    ...
    r"""Computes a radius graph for a point cloud.
    ...
    edge_index = radius(x, x, r, ptr, ptr,
