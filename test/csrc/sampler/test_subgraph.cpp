...
TEST(SubgraphTest, BasicAssertions) {
  ...
  EXPECT_TRUE(at::all(std::get<0>(out) == expected_rowptr).item<bool>());
  auto expected_col = at::tensor({1, 0, 2, 1, 3, 2}, options);
  EXPECT_TRUE(at::all(std::get<1>(out) == expected_col).item<bool>());
  auto expected_edge_id = at::tensor({3, 4, 5, 6, 7, 8}, options);
  EXPECT_TRUE(at::all(std::get<2>(out) == expected_edge_id).item<bool>());
}
