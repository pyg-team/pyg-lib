...
TEST(TemporalNeighborTypeTest, BasicAssertions) {
  auto time_options = at::TensorOptions().dtype(at::kLong);
  auto graph_options = at::TensorOptions().dtype(at::kInt);

  auto graph = cycle_graph(/*num_nodes=*/6, graph_options);
  auto rowptr = std::get<0>(graph);
  auto col = std::get<1>(graph);
  auto seed = at::arange(2, 4, graph_options);

  // Time is equal to node ID ...
  auto time = at::arange(6, time_options);
  // ... so we need to sort the column vector by time/node ID:
  col = std::get<0>(at::sort(col.view({-1, 2}), /*dim=*/1)).flatten();
  ...
  // Expect only the earlier neighbors to be sampled:
  auto expected_row = at::tensor({0, 1, 2, 3}, graph_options);
  EXPECT_TRUE(at::equal(std::get<0>(out1), expected_row));
  auto expected_col = at::tensor({2, 3, 4, 5}, graph_options);
  EXPECT_TRUE(at::equal(std::get<1>(out1), expected_col));
  auto expected_nodes =
      at::tensor({0, 2, 1, 3, 0, 1, 1, 2, 0, 0, 1, 1}, graph_options);
  EXPECT_TRUE(at::equal(std::get<2>(out1), expected_nodes.view({-1, 2})));
  auto expected_edges = at::tensor({4, 6, 2, 4}, graph_options);
  EXPECT_TRUE(at::equal(std::get<3>(out1).value(), expected_edges));
