...
def test_segment_matmul_autograd():
    ...
    assert (out[0:5] == inputs[0:5] @ other[0]).all(), 'Abs Max Err: ' + str(
        (inputs[0:5] @ other[0] - out[0:5]).abs().max())
    assert (out[5:8] == inputs[5:8] @ other[1]).all(), 'Abs Max Err: ' + str(
        (inputs[5:8] @ other[1] - out[5:8]).abs().max())
...
def test_grouped_matmul_autograd():
    ...
    assert (outs[0] == inputs[0] @ others[0]).all(), 'Abs Max Err: ' + str(
        (inputs[0] @ others[0] - outs[0]).abs().max())
    assert (outs[1] == inputs[1] @ others[1]).all(), 'Abs Max Err: ' + str(
