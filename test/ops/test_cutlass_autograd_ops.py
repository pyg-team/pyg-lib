...
def test_segment_matmul_autograd():
    ...
    assert (out[0:5] == inputs[0:5] @ other[0]).all(), 'Abs Err: ' + str(
        (inputs[0:5] @ other[0] - out[0:5]).abs())
    assert (out[5:8] == inputs[5:8] @ other[1]).all(), 'Abs Err: ' + str(
        (inputs[5:8] @ other[1] - out[5:8]).abs())
