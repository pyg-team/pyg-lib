...
CUDA_VERSION="${1:?Specify cuda version, e.g. cu128}"
...
if [ "$CUDA_VERSION" == "cu128" ]; then
    if [ ! -f "/tmp/cuda-repo-rhel8-12-8-local-12.8.1_570.124.06-1.x86_64.rpm" ]; then
    ...
    if [ ! -f "/tmp/cuda-repo-rhel8-12-6-local-12.6.3_560.35.05-1.x86_64.rpm" ]; then
    ...
    if [ ! -f "/tmp/cuda-repo-rhel8-11-8-local-11.8.0_520.61.05-1.x86_64.rpm" ]; then
